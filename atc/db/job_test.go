...
var _ = Describe("Job", func() {
	...
	Describe("DisableReruns", func() {
		Context("when the config has disable_reruns set to true", func() {
			It("returns true", func() {
				nonRerunnableJob, found, err := pipeline.Job("non-rerunnable-job")
				Expect(err).ToNot(HaveOccurred())
				Expect(found).To(BeTrue())
				Expect(nonRerunnableJob.DisableReruns()).To(BeTrue())
			})
		})

		Context("when the config does not have disable_reruns set", func() {
			It("returns false", func() {
				otherJob, found, err := pipeline.Job("some-other-job")
				Expect(err).ToNot(HaveOccurred())
				Expect(found).To(BeTrue())
				Expect(otherJob.DisableReruns()).To(BeFalse())
			})
		})
	})
